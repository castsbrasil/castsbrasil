...
describe Authorization::Oauth do
  ...
  describe '.find_or_create_by' do
    ...
    it 'should set the attributes from oauth' do
      expect(Authorization::Oauth.find_or_create_by(@oauth, user).profile_page).to be == 'profile_page'
    end

    it 'should associate the user' do
      expect(Authorization::Oauth.find_or_create_by(@oauth, user).user).to be == user
    end

    it 'should save the authorization' do
      expect(Authorization::Oauth.find_or_create_by(@oauth, user).saved).to be == true
    end

    it 'should return the authorization' do
      expect(Authorization::Oauth.find_or_create_by(@oauth, user)).to be == authorization
    end
  end
end